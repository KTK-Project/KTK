...
public:
	std::shared_ptr<Player> getGoalPlayer1() const;
	void setGoalPlayer1(const std::shared_ptr<Player> & goalPlayer1);
	std::shared_ptr<Player> getGoalPlayer2() const;
	void setGoalPlayer2(const std::shared_ptr<Player> & goalPlayer2);
	void eventResolve() const;
private:
	std::weak_ptr<Player> m_goalPlayer1;
	std::weak_ptr<Player> m_goalPlayer2;
};
